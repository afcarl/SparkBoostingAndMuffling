...
class Marvin (object):
    """
    A class that sequentially creates and aggregates classifiers to a combined model, as in the paper "Muffled Semi-Supervised Learning."
...
Example:
python marvin.py Data/covtype_binary_all.csv 10000 100 covtype 0.005 0 allrel 20 False
"""

def init_data (x_all, y_all, LABELED_SET_SIZE, UNLABEL_SET_SIZE, HOLDOUT_SET_SIZE, HOLDOUT_SET2_SIZE):
    ...
    x_train, x_unl, y_train, y_unl = train_test_split(xtrte, ytrte, test_size=UNLABEL_SET_SIZE)
    return (x_train, y_train, x_unl, y_unl, x_out, y_out, x_out2, y_out2)

def read_data (labeled_file, total_size, ytransform=False):
...
if __name__ == "__main__":
    ...
    LABELED_SET_SIZE = int(TOTAL_LABELS*0.8)
    UNLABEL_SET_SIZE = max(1000, 2*TOTAL_LABELS)
    HOLDOUT_SET_SIZE = TOTAL_LABELS - LABELED_SET_SIZE
    HOLDOUT_SET2_SIZE = 2000
    ...
    k = Marvin(DecisionTreeClassifier(max_depth=None), x_train, y_train, x_unl, unlabeled_labels=y_unl, 
